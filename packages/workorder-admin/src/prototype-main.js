  ...
  try {
    ...
    if (act === 'query') {
      if (current === 'projects') {
        ...
        const keyword = document.getElementById('keyword')?.value?.trim() || ''
        const statusSelect = document.getElementById('status-select')?.value || '全部状态'
        const typeSelect = document.getElementById('type-select')?.value || '全部类型'
        
        setSpacesFilterState(keyword, statusSelect, typeSelect)
        render()
        toast('筛选条件已应用')
      } else {
        toast('筛选条件已应用')
